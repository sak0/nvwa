						...
						if !reflect.DeepEqual(row.New, empty) {
							...
							if _, ok := oldRow.Fields["name"]; ok {
								name := oldRow.Fields["name"].(string)
                                                                fmt.Printf("should createOvsBridge:%s\n", name)
...
func displayCache(updates libovsdb.TableUpdates) {
	for table, tableUpdate := range updates.Updates {
                fmt.Printf("-------------------Cache-------------------")
		fmt.Printf("table:%v\t Update:%v\n", table, tableUpdate)
...
func (ovsdber *ovsdber)InitCache() {
	quit = make(chan bool)
	update = make(chan *libovsdb.TableUpdates)
	ovsdbCache = make(map[string]map[string]libovsdb.Row)
        initial, err := ovsdber.ovsdb.MonitorAll("Open_vSwitch", "")
        if err != nil {
                fmt.Printf("Monitor Error\n");
                return
        }
        displayCache(*initial)
